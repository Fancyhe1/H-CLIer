...
impl LicenseManager {
    ...
    fn validate_online(&self, invitation_code: &str) -> Result<ValidationResponse, String> {
        // TODO: Replace with actual license server URL
        ...
        // For demo purposes, simulate a successful response if server is unreachable
        let response = client
            .post(&format!("{}/api/v1/validate", server_url))
            .json(&request_body)
            .send();
        ...
        match response {
            ...
            Err(e) => {
                // Demo mode: simulate successful activation for testing
                // In production, this should return an error
                let is_network_error = e.is_connect() || e.is_timeout();
                if is_network_error {
                    Ok(ValidationResponse {
                        valid: true,
                        tier: Some("pro".to_string()),
                        expires_at: Some("2027-12-31T23:59:59Z".to_string()),
                        offline_grace_days: Some(7),
                        signature: Some("demo_signature".to_string()),
                        error_message: None,
                    })
                } else {
