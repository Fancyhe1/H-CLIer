...
function App() {
  ...
  const statusRightRef = useRef<HTMLDivElement>(null)
  ...
  // 状态栏溢出检测
  useEffect(() => {
    const el = statusRightRef.current
    if (!el) return
    const check = () => setStatusOverflow(el.scrollWidth > el.clientWidth)
    check()
    ...
    <ConfigProvider
      theme={{
        ...
        token: {
          ...
          colorBorder: '#252A33',
          colorText: '#E8EAED',
          colorTextSecondary: '#949BA7',
          colorTextTertiary: '#626975',
          borderRadius: 6,
        ...
        <Layout className="app-main">
          ...
          >
            <div className="toolbar-left">
              <TabBar activePanel={activePanel} onActivePanelChange={(p) => setActivePanel(p as PanelType)} />
            </div>
          ...
          <Content className="app-content">
            {/* HomePage — 无活跃会话时显示 */}
            <div style={{ display: !activeSessionId && activePanel === 'terminal' ? 'block' : 'none', width: '100%', height: '100%' }}>
              <HomePage
                onNewSession={() => {
                  const btn = document.querySelector('[data-testid="new-session-btn"]') as HTMLButtonElement
                  btn?.click()
            ...
            <div style={{ display: activePanel === 'terminal' && !!activeSessionId ? 'block' : 'none', width: '100%', height: '100%' }}>
              <MultiTerminal />
            </div>
          ...
          <div className={`status-bar ${currentTheme}`}>
            ...
            <div className={`status-right ${statusOverflow ? 'is-overflow' : ''}`} ref={statusRightRef}>
