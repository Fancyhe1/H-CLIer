...
const MENU_MARGIN = 32 // 距离窗口边缘的最小间距
...
function ContextMenu({ items, children }: ContextMenuProps) {
  ...
  useLayoutEffect(() => {
    ...
    // 先重置 max-height 让浏览器计算实际高度
    menu.style.maxHeight = ''
    const rect = menu.getBoundingClientRect()

    // 可用高度 = 窗口高度 - 上下边距
    const availH = innerHeight - MENU_MARGIN * 2
    // 菜单实际高度超出可用高度时，限制并启用滚动
    ...
    // 重新获取限制后的高度
    const finalH = menu.style.maxHeight ? availH : rect.height
    ...
    // 下边界溢出 → 菜单显示在鼠标上方
    if (y + finalH + MENU_MARGIN > innerHeight) {
      y = innerHeight - finalH - MENU_MARGIN
    }
    // 上边界溢出
    if (y < MENU_MARGIN) {
      y = MENU_MARGIN
